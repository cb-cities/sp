...
__device__ __host__ int findEdge(Vertex u, Vertex v, Edge* edges,
                                 int* weights) {
...
__global__ void Find_Vertex(Vertex* vertices, Edge* edges, int* weights,
                            int* length, int* updateLength) {
...
void printArray(int* array) {
  int i;
  for (i = 0; i < V; i++) {
    std::cout << "Shortest Path to Vertex: " << i " is " << array[i]
              << std::endl;
  }
}
...
int main(void) {
  ...
  // Len is the shortest path and updateLength is a special array for modifying
  // updates to the shortest path
